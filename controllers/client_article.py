...
def client_article_show():                                 # remplace client_index
    ...
    if "filtre_mot" in session or "filtre_min" in session or "filtre_max" in session or "filter_types" in session:
        sql = '''       SELECT Chaussure.id_chaussure AS IdChaussure,
...
;'''
    if "filtre_mot" in session:
        sql = sql + " Chaussure.nom_chaussure like %s "
        recherche = "%" + session["filtre_mot"] + "%"
        list_param.append(recherche)
        condition_and = " AND "
    if "filtre_min" in session or "filtre_max" in session:
        sql = sql + condition_and + " Chaussure.prix_de_chaussure BETWEEN %s AND %s "
...
where d.id_pointure = 1 and 
        ...
        for item in session['filter_types']:
            sql = sql + "Chaussure.id_type_chaussure = %s "
            if item != last_item:
                sql = sql + " or "
